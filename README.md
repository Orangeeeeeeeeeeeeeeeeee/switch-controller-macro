...
### 2026-08-14
- **列表回放吞键修复**:状态可视化广播改非阻塞(慢客户端不再拖慢播放导致事件突发吞键);每个宏播完回正(宏间释放,独立宏不残留状态)
- **回放兼容 66Hz**:播放摇杆节流 16ms→10ms,兼容旧 66Hz 宏(15ms 间隔),不再吞掉一半摇杆事件导致左偏;只丢 >100Hz 极密事件
- **播放回正**:宏/列表播放结束、每个循环播完都自动回正(释放全部按键 + 摇杆居中),不残留末轮状态
- **手柄状态可视化(测吞键)**:后端广播实际发送给 Switch 的状态,Web UI 虚拟手柄实时镜像——按键按下高亮、摇杆旋钮跟随;手动点击即时高亮
- **手柄摇杆顺滑 + 回中**:手柄模式 gpLoop 本地即时移动旋钮(不再卡),回中事件强制广播保证旋钮精确回正
- **摇杆中心修正**:旋钮中心坐标 35px→42px(对齐摇杆框真实中心),拖动后能完全回到正中
